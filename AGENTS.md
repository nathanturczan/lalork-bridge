...
1. Codex edits `code/firestore-bridge.js`, mirrors to root
...
2. Patch edits: the root .amxd is frozen, so either (a) Nathan unfreezes in
   Max first (snowflake toggle → save) so Codex can edit the patcher JSON
   headlessly, or (b) Codex extracts the patcher JSON from the frozen
